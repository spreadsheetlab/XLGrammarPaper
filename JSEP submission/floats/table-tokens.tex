...
EXCEL-FUNCTION & Excel built-in function name followed by `(' & (Any entry from the function list\begin{footnotesize}\textsuperscript{\number\value{footnote}}\end{footnotesize}) \textbackslash( & 5        \\
...
NR-COMBINATION & \begin{tabular}[c]{@{}l@{}} Named range which starts with \\ a string  that could be another token \end{tabular} & (TRUE $\mid$ FALSE $\mid$ [A-Z]+ [1-9][0-9]*[A-Z\textbackslash\textbackslash\_.?$\Box_1$]) [$\Box_4$]+                                                                              & 3 \\
SR-COLUMN & Column definition in structured references & [\textbackslash w\textbackslash.]+ & -3 \\ 
NUMBER & \begin{tabular}[c]{@{}l@{}}An integer, floating point\\     or scientific notation number literal\end{tabular} & [0-9]+ .? [0-9]* (e [0-9]+)? & 0 \\
REF-FUNCTION & Excel built-in reference-returning function followed by `(' & (INDEX $\mid$ OFFSET $\mid$ INDIRECT)\textbackslash( & 5 \\
REF-FUNCTION-COND & Excel built-in conditional reference function followed by `('& (IF $\mid$ CHOOSE)\textbackslash( & 5 \\
...
UDF & User Defined Function followed by `('& (\_xll\textbackslash.)? [A-Z\_\textbackslash][A-Z0-9\_\textbackslash\textbackslash.$\Box_1$]* \textbackslash ( & 4 \\
