...
An example is given in Figure \ref{figure:parsetrees}(a): the relatively simple formula \texttt{SUM(B2,5)} results in a 37-node tree with a depth of 18 nodes, while the grammar presented in this paper results in a 18-node tree with a depth of 9 nodes.
...
\subsection{Other spreadsheet software dialects}

While other spreadsheets program have generally adopted the Excel formula syntax, they sometimes differ slightly.
For example, LibreOffice uses \texttt{!} as the intersection operator instead of \texttt{\char32}.
