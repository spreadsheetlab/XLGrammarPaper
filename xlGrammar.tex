...
Secondly, because of the detail of the grammar and the large number of production rules the resulting parse trees are very complex and thus fail requirement 2.
...
The construct \texttt{[\textasciicircum$\ldots$]} must be read as any character except those specified in $\ldots$.
...
This can be confusing as whitespace does not carry meaning in the rest of the language.
An advantage of this approach is that it enables more natural language definition of intersections, e.g. \texttt{=SUM((Total_Cost Jan):(Total_Cost Apr.))}, but in our opinion this does not outweigh the disadvantages.
It is therefore unsurprising that other spreadsheet systems do not use this operator, either by using an alternative like LibreOffice which uses \texttt{!} or simply not supporting it.
