...
FUNCTION & Excel built-in function & (Any entry from the function list\footnote{A function list is available as part of the reference implementation. Lists provide by Microsoft are also available \cite{ExcelFunctionReference} and \cite{ExcelOfficialGrammar}.}) \textbackslash( & 5        & SUM( \\
...
However, this will cause a reduce-reduce conflict because the parser will have a state wherein it can reduce to both a \synt{Union} or \synt{Argument} on a \texttt{,}.
