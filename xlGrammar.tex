...
An example is given in Figure \ref{figure:parsetrees}: the relatively simple formula \texttt{SUM(B2,5)} results in a 37-node tree with a depth of 18 nodes when using the Microsoft grammar, while the grammar presented in this paper results in a 18-node tree with a depth of 9 nodes.
...
A significant amount of formula cells (286,210 or 1.28\%) invoke user-defined functions---e.g., \texttt{=[1]!erUserEmail(User_Id)}. A special case of user defined functions are the ones created using a Excel xll add-in library. These are found in the dataset invoked as \texttt{_xll.functionName} by 0.57\% of the formula cells.
...
\begin{description}
	...
	External references are normally in the form \texttt{[File]Sheet!Cell}. In 28,448 formula cells (0.13\%), however, the sheet is not specified, e.g. \texttt{=[2]!LastTrade}. These are either cases of references to external named ranges or to external UDFs.
...
While other spreadsheets program have generally adopted the Excel formula syntax, there are slight differences between programs and even Excel versions.
...
Syntactical features have also been deprecated between Excel version.
...
However, in Excel 2010 and up, regular expressions are instantly resolved---in the example, to the multiple sheet reference \texttt{=SUM(Sheet2:Sheet3!A1)}, summing up all A1 cells between Sheet2 and Sheet3, where the sheets are all sheets matching the regular expression, except the one that the formula is on.
This way, in latest versions of Excel, saved spreadsheets never contain regular expressions.
\begin{figure}
...
The grammar does not support the use of labels, and it would mistakenly parse them as named ranges.
...
While we have empirically shown a high compatibility in terms of successful parse rate, we do not have as much evidence that the produced parse trees are correct as this is only tested by usage of and unit tests in the reference implementation.
However, we believe it is unlikely that a formula can be parsed with the presented grammar but would be interpreted differently by Excel.
